...
std::string Pnet::filename = "./Pnet.txt";

void Pnet::setFileName(const char* pnet_model)
{
        filename = std::string(pnet_model);
        cout << "model path: " << filename << endl;
}

//Pnet::Pnet(std::string filename){
//        Pthreshold = 0.6;
//        nms_threshold = 0.5;
//        firstFlag = true;
//        this->rgb = new pBox;
...
Pnet::Pnet(){
        ...
        if (Pnet::filename.empty()) {
                cout << "model is empty!" << endl;
                exit(-1);
        }
        readData(Pnet::filename, dataNumber, pointTeam);
}
Pnet::~Pnet(){
        freepBox(this->rgb);
        freepBox(this->conv1);
...
mtcnn::mtcnn(const char*pnet_model, const char* rnet_model, 
                const char* onet_model, int row, int col) : refineNet(rnet_model), outNet(onet_model)
{
        Pnet::setFileName(pnet_model);  // set pnet model path
...
std::vector<int> mtcnn::detect(const char * imagePath)
{
        ...
        for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
                if((*it).exist){
                        rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2), Scalar(0,0,255), 2,8,0);
                        results.push_back((*it).x1);
                        results.push_back((*it).y1);
                        results.push_back((*it).x2);
                        results.push_back((*it).y2);
                        for(int num=0;num<5;num++)circle(image,Point((int)*(it->ppoint+num), (int)*(it->ppoint+num+5)),3,Scalar(0,255,255), -1);
                }
        }
